...
interface ListsDashboardPageProps {
  ...
  onComparePrices: (listId: string) => void
}

import SortButton from './SortButton'

const ListsDashboardPage: React.FC<ListsDashboardPageProps> = ({
  lists,
  onCreateNew,
  onSelectList,
  onDeleteList,
          ...
          <div className="flex flex-1 justify-end gap-4 sm:gap-6">
            <label className="hidden sm:flex flex-col min-w-40 !h-10 max-w-64">
              <div className="flex w-full flex-1 items-stretch rounded-lg h-full">
                ...
                <input
                  className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-zinc-900 dark:text-zinc-100 focus:outline-0 focus:ring-0 border-none bg-zinc-100 dark:bg-zinc-900 focus:border-none h-full placeholder:text-zinc-500 dark:placeholder:text-zinc-400 px-4 rounded-l-none border-l-0 pl-2 text-sm font-normal leading-normal"
      ...
      <main className="px-4 sm:px-8 md:px-10 flex flex-1 justify-center py-5 sm:py-8">
        <div className="layout-content-container flex flex-col max-w-7xl flex-1 gap-6">
          <div className="flex flex-wrap justify-between items-center gap-4">
            <h1 className="text-zinc-900 dark:text-zinc-100 text-3xl sm:text-4xl font-black tracking-tighter">
              Suas Listas
            </h1>
            <button
              onClick={onCreateNew}
              className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-background-dark text-sm font-bold leading-normal tracking-wide gap-2"
            >
          ...
          <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 -mx-4 sm:-mx-8 md:-mx-10 px-4 sm:px-8 md:px-10">
            <SortButton
              ...
              isActive={sortOption === SortOption.Name}
              direction={sortDirection}
            />
            <SortButton
              label="Ordenar por Data de Criação"
              icon="calendar_today"
              onClick={() => handleSort(SortOption.CreationDate)}
              isActive={sortOption === SortOption.CreationDate}
              direction={sortDirection}
            />
            <SortButton
              label="Ordenar por Última Modificação"
              icon="edit_calendar"
              onClick={() => handleSort(SortOption.LastModified)}
              isActive={sortOption === SortOption.LastModified}
              direction={sortDirection}
            />
          </div>
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
              {filteredAndSortedLists.map((list) => (
                <ListCard
                  ...
                  onShop={onStartShopping}
                  onCompare={onComparePrices}
                />
              ))}
            </div>
          ) : (
